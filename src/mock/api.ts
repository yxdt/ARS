...
const messages = [
  {
    ...
    ordNo: '1',
    cdc: '0101',
    sentTime: new Date(new Date().valueOf() - 3 * 24 * 60 * 60 * 1000).toLocaleDateString(),
    status: 0,
  ...
  },
  {
    ...
    ordNo: '2',
    cdc: '0001',
    sentTime: new Date(new Date().valueOf() - 4 * 24 * 60 * 60 * 1000).toLocaleDateString(),
    status: 0,
  ...
  },
  {
    ...
    ordNo: '1',
    cdc: '0101',
    sentTime: new Date(new Date().valueOf() - 2.5 * 24 * 60 * 60 * 1000).toLocaleDateString(),
    status: 0,
    toOpenid: 'sjlajdfaslkfda',
  },
  {
...
const statusList: Array<wbStatus> = [
  { status: 1, doneDate: new Date(), caption: '确认到达', comment: '司机：1383832003', seq: 0 },
  { status: 2, doneDate: new Date(), caption: '回执上传完成', comment: '合计：3张', seq: 1 },
  { status: 4, doneDate: null, caption: '回执审核通过', comment: '审核中', seq: 2 },
...
async function markMsgRequest(url: string) {
  const msgid = url.substr(url.indexOf('msgid=') + 6, url.indexOf('&') - url.indexOf('msgid=') - 6);
...
async function queryMsgRequest(queryData: msgQueryParams): Promise<TimsResponse<msgQueryData>> {
...
function msgRequest(url: string) {
  const wbno = url.substr(url.indexOf('wbno=') + 5, url.indexOf('&') - url.indexOf('wbno=') - 5);
...
function photosRequest(url: string) {
  ...
  const photos: photoListData = {
    photos: pics.filter((item) => item.indexOf('_' + wbno + '_') > 0).map((item, index) => ({ url: item, status: index % 3 })),
  };
  if (wbno === '999') {
    success = false;
  }
