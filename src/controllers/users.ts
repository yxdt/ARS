...
async function doOpenidLogin(openId): Promise<loginResult> {
  ...
  if (restRet.code === '0000') {
    if (restRet.data && restRet.data.data && restRet.data.data.username) {
      const roleName = restRet.data.departs && restRet.data.departs.length > 0 ? restRet.data.departs[0].departName : restRet.data.data.realname;
      ret.user = {
...
export { checkToken, getDriverLocation, getWxOpenId, getUserInfo, doLogin, doOpenidLogin, getDriverInfo };
