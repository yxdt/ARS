...
import { loadWaybill, confirmArrive, confirmComplete } from '../../controllers/waybill';

import { approvePicture, rejectPicture, deletePicture } from '../../controllers/camera';
...
export default class Index extends Component<null, SheetState> {
  constructor() {
    ...
    const today = Today.getFullYear() + '-' + (Today.getMonth() < 9 ? '0' + (Today.getMonth() + 1) : Today.getMonth() + 1 + '');
    ...
    loadWaybill(wbno)
      .then((ret: WaybillResult) => {
        if (ret.result === 'success') {
          ...
          const arrDate = ret.waybill.arriveTime ? ret.waybill.arriveTime.split('.')[0] : '';
          Taro.setStorage({
            key: 'waybillStatus',
        ...
        if (ret.result === 'success') {
          ...
          if (pidx === '1') {
            ...
            for (let i = 0; i < wb.length; i++) {
              if (wb[i].wbNum === this.state.waybill.wbNum + this.state.waybill.shiptoCode) {
                wb[i].statusNum = 8;
  ...
  driverConfirmArrive() {
    ...
    if (this.state.rdcNum === this.state.waybill.shiptoCode) {
      ...
      confirmArrive(this.state.waybill.wbNum, this.state.waybill.shiptoCode, this.state.cellphone, this.state.confirmTime)
        .then((ret) => {
          if (ret.result === 'success') {
            this.setState({
              arrived: true,
        ...
        {preview ? (
          <View className="preview-span">
            ...
            <View style="display:flex; flex-direction:row">
              {(selCaption === '已上传' && waybill.statusNum === 3) || waybill.statusNum === 1 || (isSuper && waybill.statusNum < 8) ? (
                <Button
                  className="preview-confirm-button"
                  onClick={() => {
                    deletePicture(waybill.wbNum + waybill.shiptoCode, this.state.selPicId, Taro.getStorageSync('userOpenId')).then((ret) => {
              ...
              {selCaption !== '已上传' || !isSuper ? null : (
                <View style="display:flex; flex-direction:row;flex:4">
                  <Button
                    ...
                    onClick={() => {
                      approvePicture(waybill.wbNum + waybill.shiptoCode, this.state.selPicId, Taro.getStorageSync('userOpenId'))
                        .then((ret) => {
                          if (ret.result === 'success') {
                            Taro.atMessage({
                              message: '照片审核通过',
            ...
            <AtModal isOpened={this.state.confirmReject}>
              ...
              <AtModalAction>
                ...
                <Button
                  ...
                  className={!remark || remark.trim().length <= 0 ? 'disabled-semi-right' : 'home-input-semi-right'}
                  onClick={() => {
                    rejectPicture(waybill.wbNum + waybill.shiptoCode, selPicId, this.state.remark, Taro.getStorageSync('userOpenId'))
                      .then((ret) => {
                        this.setState({ confirmReject: false });
                        if (ret.result === 'success') {
                          Taro.atMessage({
                          ...
                          for (let i = 0; i < waybill.photos.length; i++) {
                            if (waybill.photos[i].id === this.state.selPicId) {
                              waybill.photos[i].caption = '驳回:' + this.state.remark;
                            }
                          }
                          this.setState({ preview: false });
          ...
          <View>
            <View className="sheet-detail-index">
              ...
              <View className="sheet-info-span">
                <View className="form-caption-split">
                  ...
                  <Text className={confirmed ? 'arrived form-item' : 'notarrive form-item'}>{confirming ? '更新中...' : waybill.statusCaption}</Text>
                </View>
                <View className="form-caption-split">
                  ...
                  <View className="form-detail-span">
                    <View className="form-detail-header">
                      ...
                      <View className="form-detail-title-right">
                        {(!confirmed || isSuper) && (waybill.statusNum < 8 || arriveTimeStr.length > 0) ? (
                          <AtButton
                ...
                <View className="form-detail-span">
                  ...
                  <ShipItems current={0} pageCount={waybill.maxPage} shipItems={waybill.shipItems} />
                </View>
                ...
                <AtButton
                  ...
                  onClick={() => {
                    ...
                    if (pidx === '1') {
                      ...
                      for (let i = 0; i < wb.length; i++) {
                        if (wb[i].wbNum === waybill.wbNum + waybill.shiptoCode) {
            ...
            <AtModal isOpened={confirmedComplete}>
              ...
              <AtModalContent>
                <View className="toast-main">
                  <View className="confirm-info-1">注意：确认后，该运单将直接标记为“IOD完成状态”。</View>
                  <View className="confirm-info-1">当前运单的全部已上传回执将被标记为“通过”。</View>
                </View>
              </AtModalContent>
              <AtModalAction>
                <Button
                ...
                <Button className="home-input-semi-right" onClick={this.superConfirmComplete}>
                  确认
                </Button>
              </AtModalAction>
            </AtModal>
            {waybill.statusNum > 0 ? null : (
              <AtModal isOpened={confirmedArrive}>
                ...
                <AtModalAction>
                  ...
                  <Button className="home-input-semi-right" onClick={this.driverConfirmArrive}>
                    确认到达
                  </Button>
                </AtModalAction>
              </AtModal>
