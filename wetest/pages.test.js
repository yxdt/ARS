...
describe('Index page test', () => {
  ...
  it('index page should have elements', async () => {
    ...
    const btn = await page.$('.home-button view');
    //console.log('index.btn:', btn);
    const btnText = await btn.text();
    expect(btnText).toBe('获取交货单信息');

    const btnIcon = await page.$('.home-button view text');
  ...
  it('should display error prompt', async () => {
    ...
    //const btn = await btnctn.$('.at-button--text');
    ...
    const btn1 = await page.$('.home-button');
...
describe('verify uploaded receipts test', () => {
  ...
  it('should have verify permission and have uploaded list', async () => {
    ...
    const listItems = await page.$$('.at-list__item');
    ...
    //通过后，列表应该少一个
    //page = await mPrg.currentPage();
    const listItems1 = await page.$$('.at-list__item');
    ...
    //获取按钮,并点击确认退回
    const btnsa2 = await page.$('.at-modal__action');
    ...
    //退回操作后，待确认列表再次减一
    const listItems2 = await page.$$('.at-list__item');
  ...
  it('should have verify permission and have query function', async () => {
    ...
    expect(dates1).toBe(new Date(new Date().valueOf() - 7 * 24 * 60 * 60 * 1000).toLocaleDateString().replace('-', '/'));
    ...
    //应为5条记录
    let results = await qryResult.$$('.at-list__item');
